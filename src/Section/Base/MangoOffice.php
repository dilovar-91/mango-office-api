...
Class MangoOffice {
    ...
    function sendSms($from, $to_number, $text, $sms_sender=null, $command_id = null) {
        $data = [
            ...
            'to_number' => $to_number,
            "text" => $text, // текст смс
            "sms_sender" => $sms_sender // ОБЯЗАТЕЛЬНЫЙ ПАРАМЕТР. имя отправителя. Если не заполнено - будет использоваться имя отправителя, выбранное в ЛК.
        ];
        //if (!is_null($from) && !is_null($to_number) && !is_null($text)) {
        //    $data['from']['number'] = $number;
        //}
